...
import org.example.distributedorchestration.worker.application.port.TaskExecutor;
...
public final class DefaultTaskExecutor implements TaskExecutor {
    ...
    @Override
    public void execute() {
        log.debug("Default executor execute start taskId={} payloadLength={}", taskId, forwardPayload.length());
        if (taskId.isBlank()) {
    ...
    public void compensate() {
        log.debug("Default executor compensate start taskId={} payloadLength={}", taskId, compensationPayload.length());
        if (taskId.isBlank()) {
            throw new IllegalArgumentException("task_id is blank");
        }
