...
import io.github.ks22ss.distributedorchestration.worker.application.service.DefaultTaskExecutor;
...
/** gRPC worker: maps transport requests to worker commands and executes them. */
@Component
public class WorkerGrpcService extends WorkerServiceGrpc.WorkerServiceImplBase {
        ...
        try {
            ...
            ExecuteTaskCommand command =
                    new ExecuteTaskCommand(
                            request.getTaskId(), request.getPayload() == null ? "" : request.getPayload());
    ...
    public void compensateTask(
            CompensationRequest request, StreamObserver<TaskResponse> responseObserver) {
        try {
            log.debug(
                    "Worker compensate request received workflowId={} taskId={}",
                    request.getWorkflowId(),
                    request.getTaskId());
            CompensateTaskCommand command =
                    new CompensateTaskCommand(
                            request.getTaskId(),
                            request.getCompensationPayload() == null ? "" : request.getCompensationPayload());
            DefaultTaskExecutor.forCompensation(command).compensate();
            log.info(
                    "Worker compensate success workflowId={} taskId={}",
                    request.getWorkflowId(),
                    request.getTaskId());
            responseObserver.onNext(successResponse());
        } catch (Exception e) {
