...
/**
 * gRPC clients to workers. Multiple comma-separated {@code orchestration.worker.endpoints} values build a round-robin
 * {@link WorkerStubPool}; when unset, {@code host} and {@code grpc-port} define a single target.
 */
@Configuration
public class OrchestratorGrpcClientConfiguration {
    ...
    public WorkerStubPool workerStubPool(
        ...
        for (HostPort hp : targets) {
            ManagedChannel channel = NettyChannelBuilder.forAddress(hp.host(), hp.port()).usePlaintext().build();
    ...
    private static List<HostPort> parseTargets(String endpointsCsv, String defaultHost, int defaultPort) {
        ...
        for (String raw : endpointsCsv.split(",")) {
            ...
            if (colon <= 0 || colon == part.length() - 1) {
                throw new IllegalArgumentException("Invalid worker endpoint (expected host:port): \"" + part + "\"");
            }
