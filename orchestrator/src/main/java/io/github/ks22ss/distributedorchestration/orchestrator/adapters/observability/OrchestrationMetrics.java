package io.github.ks22ss.distributedorchestration.orchestrator.adapters.observability;
...
/** Metrics: task execution time, retry counts, and terminal outcomes. */
@Component
public class OrchestrationMetrics {
    ...
    public OrchestrationMetrics(MeterRegistry registry) {
        ...
        this.dispatchRetryAttempts =
                Counter.builder("orchestration.task.retry.attempts")
                        .description(
                                "Dispatch retry attempts after a failed execute attempt (before terminal failure)")
                        .register(registry);
        this.dispatchSuccess =
                Counter.builder("orchestration.task.dispatch.success")
                        .description("Tasks that reached SUCCESS after dispatch")
                        .register(registry);
        this.dispatchFailure =
                Counter.builder("orchestration.task.dispatch.failure")
                        .description(
                                "Tasks that exhausted dispatch retries (terminal); pair with success for failure_rate in Prometheus")
                        .register(registry);
        this.compensationRetryAttempts =
                Counter.builder("orchestration.compensation.retry.attempts")
                        .description("Compensation RPC retry attempts")
                        .register(registry);
        this.compensationSuccess =
                Counter.builder("orchestration.compensation.success")
                        .description("Successful compensateTask completions")
                        .register(registry);
        this.compensationFailure =
                Counter.builder("orchestration.compensation.failure")
