...
/** Persists dispatch outcomes in short transactions; backoff is deferred via {@code next_retry_at}. */
@Service
...
public class WorkerDispatchPersistence {
    ...
    /**
     * Claims a {@link TaskStatus#PENDING} task for dispatch: sets {@code next_retry_at} to a short lease so the runnable
     * selector and retry scanner skip it until success, failure rescheduling, or lease expiry (crash safety).
     */
    ...
    /**
     * Applies spec Step 9: {@code delay = (int) Math.pow(2, retryCount)} using the count <em>before</em> increment, then
     * increments {@code retry_count}. Marks {@link TaskStatus#FAILED} when retries are exhausted; otherwise returns task
     * to {@link TaskStatus#PENDING} and sets {@code next_retry_at}.
     *
